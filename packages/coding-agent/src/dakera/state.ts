...
export class DakeraSessionState {
	...
	async #ensureSessionRegistered(): Promise<void> {
		...
		this.#sessionRegisteredFor = this.sessionId;
		try {
			const serverId = await this.client.sessionStart(this.agentId, this.sessionId, {
				source: "omp",
	...
	async #publishTranscript(messages: HindsightMessage[], republish: boolean): Promise<void> {
		...
		const [id] = await this.#store([{ content, memoryType: "episodic", importance: this.config.retainImportance }]);
	 ...
	 * session disposal. Failed close is non-fatal and must not mask the
	 * pending-drain step that follows it.
	 */
	async endSessionWithSummary(summary: string): Promise<void> {
		try {
			await this.client.sessionEnd(this.agentId, this.sessionId, summary);
		} catch (err) {
			if (this.config.debug) {
